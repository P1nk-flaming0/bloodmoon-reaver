...
- Declining or uninstalling the Bloodmoon Reaver kit (`DESIGNATED 100`) no longer leaves Dynamic Portrait Switching (`DESIGNATED 200`) available or installed on its own.
...
- Component titles now use TRA stringrefs; both components have stable WeiDU `DESIGNATED` IDs (`100`, `200`) and Project Infinity `LABEL`s (`PF#-bloodmoon-reaver-kit`, `PF#-bloodmoon-reaver-dynamic-portrait-switching`).
